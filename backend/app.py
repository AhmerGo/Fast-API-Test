...
async def chat(request: ChatRequest):
    ...
    if not user_input:
        ...
        raise HTTPException(
            status_code=400,  # Use the appropriate status code
    ...
    if result:
        ...
        raise HTTPException(
            status_code=500,  # Use the appropriate status code
            detail="Failed to process chat"
        )
    
@app.post("/save_chat")
async def save_chat(request: SaveChatRequest):
    user_id = request.user_id  # Assuming you've added user_id to the SaveChatRequest model
